    ...
    <div class="max-w-4xl mx-auto my-10 px-2 md:px-10">
        <div class="justify-between items-start">
            <div class="flex justify-between items-start">
                <div class="flex">
                    <div class="text-xl font-bold md:mr-5">スクール一覧</div>
                    <div class="hidden lg:flex">
                        <form action="{{ route('admin.school.search') }}" method="get">
                            <div class="flex">
                                <div class="flex items-center rounded-md px-2 bg-white mr-2 lg:mr-5 w-32 md:w-48">
                                    ...
                                    type="text" value="{{ $target }}" placeholder="スクールを検索" />
                                ...
                                <div>
                                    <select onchange="this.form.submit()" name="sort"  class="rounded-md border-none text-gray-600 text-xs md:text-sm py-2 px-2 md:px-4 leading-tight focus:outline-none foucus:border-none foucus:appearance-none focus:ring-0 w-32 md:w-48">
                                        ...
                                        <option value="name" @if($sort  === "name") selected  @endif>スクール名</option>
                ...
                <div class="">
                    <a href="{{ route("admin.school.create") }}">
                       <x-button.primary-button class="py-1 md:w-40">新規登録</x-button.primary-button>
                    </a>
                </div>
            </div>
        ...
        <div class="mt-5">
            <table class="mt-2 w-full">
                <tr>
                    <th class="border-r-2 border-gray-100 bg-gray-400 text-white text-xs p-1">スクール名</th>
                ...
                <tr  class="border-b-2 border-gray-300">
                    <td class="w-3/12 text-xs p-1">{{ $school->name }}</td>
                    <td class="w-5/12 md:w-6/12 text-xs p-1">{{ $school->address }}</td>
                    <td class="w-1/12 text-xs p-1 text-center">{{ $school->gr }}</td>
                    <td class="w-1/12 text-xs p-1">{{ $school->updated_at->format('y/m/d') }}</td>
            ...
            {{-- データが登録されていない時 --}}
            @if (count($schools)===0)
                <div class="text-center font-bold mt-5 text-gray-500">スクールは登録されていません</div>
            @endif
        </div>
    </div>

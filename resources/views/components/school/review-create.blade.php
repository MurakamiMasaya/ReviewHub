    ...
    <div class="mt-5 md:mt-10">
        <div class="my-5 py-10 px-5 shadow-xl">
            ...
            <form action="{{ route('school.review.confilm', ['school'=>$school->id]) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="mt-10 text-sm md:text-md lg:text-lg font-bold">
                    本文(必須) : 
                    <textarea name="review" rows="6" value="{{ old('review') }}" required maxlength="140" class="shadow appearance-none border w-full rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" placeholder="1ヶ月でフルスタックエンジニアになれました！">{{ old('review') }}</textarea>

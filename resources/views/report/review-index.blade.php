<x-app-layout>
    ...
    <div class="max-w-lg mx-auto my-10 px-5 md:px-10">
        ...
        <div class="max-w-lg mx-auto">
            ...
            <form method="post" action="{{ route('review.report.post') }}" class="mt-10">
                @csrf
                <input type="hidden" name="review_id" value="{{ $reviewDetail->id }}">
                <input type="hidden" name="type" value="{{ $type }}">

                <div class="mt-3 text-sm md:text-md lg:text-lg font-bold">
                    本文 : 
                    <div  class="shadow appearance-none border w-full rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{!! nl2br(e($reviewDetail->review)) !!}</div>
                </div>

                <div class="mt-5 text-sm md:text-md lg:text-lg font-bold">
                    ユーザーネーム : 
                    <div class="shadow appearance-none border w-full rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ $reviewDetail->user->username }}</div>
                </div>
